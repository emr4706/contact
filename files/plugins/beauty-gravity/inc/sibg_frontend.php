...
class sibg_frontend {
    ...
    public function GravityInit(){
		...
		// add_action( 'wp_enqueue_scripts', array($this,'sibg_enqueue_scripts') );
	...
	public function sibg_enqueue_scripts($form) {
		...
		$theme   = $options['form_theme'] ? $options['form_theme'] : "Default";
		if($theme!="Default"){
        ...
        if ($animation != "None"){
			...
			<script>
				if(typeof sibg_add_submit_on_click === 'undefined')
					...
					sibg_add_submit_on_click[<?= $form['id'] ?>] = true;
			</script>
			<?php
    ...
    public function AddStyleClass($form_tag , $form) {
		...
		$tags      = "data-color='$mainColor' data-animation='$animation' is_rtl='$is_rtl' is_ux='$is_ux'";
        $form_tag  = substr_replace( $form_tag, $tags."' id", $start, "2" );
        ...
        //gravityforms <form> tag has property class
    ...
    public function FormCustomization($content, $field, $value, $lead_id, $form_id){
        ...
        if ($customize["form_theme"] == "BG_Android"){
			...
			if ($field->is_tooltip && $tooltipTheme != "None") {
				...
				$start = strpos($content, "<label");
				$content = substr_replace($content, '<div class="bg_label_container">', $start, 0);
				$finish = strpos($content, "</label>", $start);
				$content = substr_replace($content, "</label></div>", $finish, 8);
			}
	...
	public function add_dynamic_style(){
		echo "<style>$this->color_style</style>";
	}
	...
	public function add_class_to_form($field, $form_id, $customize){
		...
		$content = "<input type='hidden' class='sibg_form_init' value='gform_$form_id' data-class='$classes' data-color='$mainColor' data-animation='$animation' is_rtl='$is_rtl' is_ux='$is_ux'>";
		return $content;
	}
    ...
    public static function GetCustomizeSetting($form_id){
        ...
        if(!class_exists("sibg_frontend_pro")) {
