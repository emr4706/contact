...
class GF_Field_Mollie extends GF_Field_CreditCard {
	...
	public function get_form_editor_field_icon() {
		return gf_mollie()->get_base_url() . '/images/menu-icon.svg';
	}
	...
	public function is_value_submission_empty( $form_id ) {
		$payment_method = rgpost( 'input_' . $this->id . '.6' );
	...
	public function get_field_input( $form, $value = array(), $entry = null ) {
		...
		if ( ! gf_mollie()->initialize_api() ) {
			if ( ! $is_admin ) {
				return sprintf( esc_html__( '%sPlease check your Mollie Add-On settings. Your account is not connected yet.%s' ), '<div class="gfield_description validation_message">', '</div>' );
			} else {
		...
		if ( empty( $this->choices ) ) {
			...
			if ( is_admin() ) {
				...
				return '<div class="gfield_description validation_message">' . $message . '</div>';
			}
		}

		if ( ! $is_admin && ! gf_mollie()->has_feed( $form['id'] ) ) {
			return sprintf( esc_html__( '%sPlease check if you have activated a Mollie feed for your form.%s' ), '<div class="gfield_description validation_message">', '</div>' );
		}
		...
		if ( $is_sub_label_above ) {
			$card_field = "<span class='ginput_full{$class_suffix}' id='{$field_id}_1_container'>
                                    ...
                                    <span id='{$field_id}_1' class='ginput_card_field ginput_card_number'></span>
                                 </span>";
		} else {
			$card_field = "<span class='ginput_full{$class_suffix}' id='{$field_id}_1_container'>
                                    {$card_icons}
		...
		if ( $is_sub_label_above ) {
			$expiration_field = "<span class='ginput_full{$class_suffix} ginput_cardextras' id='{$field_id}_2_container'>
                                            <span class='ginput_cardinfo_left{$class_suffix}' id='{$field_id}_2_cardinfo_left'>
                                                ...
                                                <span id='{$field_id}_2' class='ginput_card_field ginput_card_expiration'></span>
                                            </span>";
		...
		if ( $is_sub_label_above ) {
			$security_field = "<span class='ginput_cardinfo_right{$class_suffix}' id='{$field_id}_2_cardinfo_right'>
                                                ...
                                                <span id='{$field_id}_3' class='ginput_card_field ginput_card_security_code'></span>
		...
		if ( $is_sub_label_above ) {
			$card_name_field = "<span class='ginput_full{$class_suffix}' id='{$field_id}_5_container'}>
                                            ...
                                            <span id='{$field_id}_5' class='ginput_card_field ginput_card_name'></span>
                                        </span>";
		} else {
			$card_name_field = "<span class='ginput_full{$class_suffix}' id='{$field_id}_5_container'}>
                                            <span id='{$field_id}_5' class='ginput_card_field ginput_card_name'></span>
		...
		$field_input = $payment_method_field . $field_note . "<div class='ginput_complex{$class_suffix} ginput_container ginput_container_mollie_components' id='{$field_id}' $style>" . $card_field . $expiration_field . $security_field . $card_name_field . ' </div>';
	...
	public function get_field_content( $value, $force_frontend_label, $form ) {
		...
		if ( ! GFCommon::is_ssl() && ! $is_admin ) {
			$field_content = "<div class='gfield_creditcard_warning_message'><span>" . esc_html__( 'This page is unsecured. Do not enter a real credit card number! Use this field only for testing purposes. ', 'gravityformsmollie' ) . '</span></div>' . $field_content;
		}
	...
	public function get_field_label_class() {
		return 'gfield_label gfield_label_before_complex gfield_label_mollie';
	}
